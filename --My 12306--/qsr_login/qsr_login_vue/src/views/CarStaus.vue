<template>
    <div style="padding: 10px"> <!--给表格和其他空一点间隙--->
        ...
        <el-table :data="tableData"
            ...
            <el-table-column prop="carStaus" label="列车状态" />
            <el-table-column prop="carTstaus" label="列车状态详情" />
            ...
            <el-dialog :visible.sync="dialogFormVisible" width="30%" title="提示">
                ...
                <el-form :model="form" label-width="120px">
                    ...
                    <el-form-item label="列车状态">
                        <el-input v-model="form.carStaus" /> <!--数据库中如下划线nick_name在vue中写成小驼峰形式 不然会报400或500的错--->
                    </el-form-item>
                    <el-form-item label="列车状态详情">
                        <el-input type="textarea" v-model="form.carTstaus" />
                    </el-form-item>
                </el-form>
                <template #footer>
        ...
        methods:{
            ...
            save(){
                if(this.form.id) { //更新 有该id(主键)
                    request.put("/carstaus", this.form).then(res => {
                     ...
                     * */
                    //请求后台接口 请求参数
                    request.post("/carstaus",this.form).then(res =>{
            ...
            handleDelete(id){
                console.log(id)
                request.delete("/carstaus/" + id).then(res => {
                    if(res.code ==='0'){
                        this.$message({
                            type: "success",
                            message: "删除成功"
