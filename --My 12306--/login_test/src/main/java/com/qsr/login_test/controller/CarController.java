...
import com.qsr.login_test.entity.Car;
import com.qsr.login_test.mapper.CarMapper;
...
public class CarController {
    ...
    {
        LambdaQueryWrapper<Car> wrapper = Wrappers.<Car>lambdaQuery(); //通过lambdaQuery生成一个wrapper对象
        if(StrUtil.isNotBlank(search)){ //引入hutool工具类 判断search不是空 避免nickName为空无法查出
            wrapper.like(Car::getCarNumber,search); //模糊查询以昵称为准
        }
